...
class multiTimeAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask=None, dropout=None):
        ...
        scores = scores.unsqueeze(-1).repeat_interleave(dim, dim=-1) # 与value的最后一维度相同对齐 [32, 8, 48, 120, 60]
        if mask is not None: 
            if len(mask.shape)==3:
                mask=mask.unsqueeze(-1) # [Batch_size, 1,120, 60]

            scores = scores.masked_fill(mask.unsqueeze(-3) == 0, -10000) # [32, 8, 48, 120, 60] softmax后为0
