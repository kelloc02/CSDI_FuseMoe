...
def parse_id(id_, missing_ratio=0.1):
    data = pd.read_csv("./data/ihm_data_test/{}.txt".format(id_))
    # set hour
...
def get_idlist():
    patient_id = []
    for filename in os.listdir("./data/ihm_data_test"):
...
class Physio_Dataset(Dataset):
    def __init__(self, eval_length=48, use_index_list=None, missing_ratio=0.0, seed=0):
        ...
        self.patient_ids = []  # 存储患者ID列表

        path = (
            "./data/ihm_missing_withid_test" + str(missing_ratio) + "_seed" + str(seed) + ".pk"
        )
        ...
        if os.path.isfile(path) == False:  # if datasetfile is none, create
            ...
            for id_ in idlist:
                try:
                    ...
                    self.patient_ids.append(id_)  # 添加患者ID
...
###
            ...
            self.observed_values = np.clip(self.observed_values, -1.0, 1.0)
            ...
            with open(path, "wb") as f:
                pickle.dump(
                    [self.observed_values, self.observed_masks, self.gt_masks, self.patient_ids], f
                )
        else:  # load datasetfile
            with open(path, "rb") as f:
                self.observed_values, self.observed_masks, self.gt_masks ,self.patient_ids= pickle.load(
                    f
                )
    ...
    def __getitem__(self, org_index):
        ...
        s = {
            ...
            "patient_id": self.patient_ids[index],  # 返回患者ID
        }
        return s
...
def get_dataloader_all(seed=1, batch_size=16, missing_ratio=0):
    ...
    # 构建整个数据集（不传 use_index_list）
    dataset_all = Physio_Dataset(
        missing_ratio=missing_ratio,
        seed=seed,
        use_index_list=None  # None 表示使用全部样本
    )

    # 所有样本索引（0 ~ N-1）
    indlist = np.arange(len(dataset_all))

    # 若需要显式指定 use_index_list，可重新构造（可省略）
    dataset_all.use_index_list = indlist
