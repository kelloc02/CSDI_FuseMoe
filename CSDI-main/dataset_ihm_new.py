...
def parse_id(id_, missing_ratio=0.1):
    data = pd.read_csv("./data/ihm_data_train_new/{}.txt".format(id_))
    # set hour
...
def get_idlist():
    patient_id = []
    for filename in os.listdir("./data/ihm_data_train_new"):
...
class Physio_Dataset(Dataset):
    def __init__(self, eval_length=48, use_index_list=None, missing_ratio=0.0, seed=0):
        ...
        self.gt_masks = []
        path = (
            "./data/ihm_missing_new_all" + str(missing_ratio) + "_seed" + str(seed) + ".pk"
        )
