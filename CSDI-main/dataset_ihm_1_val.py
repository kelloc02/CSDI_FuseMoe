...
def parse_id(id_, missing_ratio=0.1):
    data = pd.read_csv("./data/ihm_data_val/{}.txt".format(id_))
    # set hour
...
def get_idlist():
    patient_id = []
    for filename in os.listdir("./data/ihm_data_val"):
...
class Physio_Dataset(Dataset):
    def __init__(self, eval_length=48, use_index_list=None, missing_ratio=0.0, seed=0):
        ...
        path = (
            "./data/ihm_missing_withid_val" + str(missing_ratio) + "_seed" + str(seed) + ".pk"
        )
